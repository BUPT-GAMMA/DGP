...
def get_ood_dataset(args, train_per=0.8, need_str_enc=False):
    ...
    if TU:
        ...
        dataset_aug = TUDataset_aug(path, name=DS, transform=None, aug=args.aug)
    ...
    try:
        ...
        if TU:
            ...
            dataset_num_features = dataset.data.x.shape[1] if hasattr(dataset.data, 'x') else 1
    ...
    data_list_train2 = []
    idx = 0
    for data, data_aug_tuple in zip(dataset_train, dataset_train_aug):
        data.y = 0
        data['idx'] = idx
        data = data.to(device)
        ...
        if TU:
            data_aug = data_aug_tuple[1]
        else:
            data_aug = data_aug_tuple
        data_aug.y = 0
        data_aug['idx'] = idx
        data_aug = data_aug.to(device)
        data_list_train2.append((data, data_aug))
        idx += 1

    dataloader = DataLoader(data_list_train, batch_size=batch_size, shuffle=True)
    dataloader2 = DataLoader(data_list_train2, batch_size=batch_size, shuffle=True)
    ...
    dataloader_valid = DataLoader(data_list_valid, batch_size=batch_size, shuffle=False)
    ...
    for data in dataset_test_ood:
        data.y = 1
        data.edge_attr = None
        data = data.to(device)
        data_list_test.append(data)

    dataloader_test = DataLoader(data_list_test, batch_size=batch_size, shuffle=False)

    return dataloader, dataloader_valid, dataloader_test, dataloader2, dataset_num_features
